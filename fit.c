...
fitted_splines(pixel_outline_list_type pixel_outline_list,
    ...
    for (this_list = 0; this_list < CURVE_LIST_ARRAY_LENGTH (curve_array);
        ...
        if (notify_progress)
            notify_progress((((at_real) this_list) / ((at_real) CURVE_LIST_ARRAY_LENGTH (curve_array) * (at_real) 3.0) +
                             (at_real) 0.333),
                            progress_data);
    ...
    for (this_pixel_o = 0; this_pixel_o < O_LIST_LENGTH (pixel_list);
        ...
        if (corner_list.length == 0) { /* No corners.  Use all of the pixel outline as the curve.  */
            ...
            if (!pixel_o.open) {
                ...
                if (last_curve) NEXT_CURVE(last_curve) = NULL;
            }
        }
            ...
            if ((cross_curve == true) || (CURVE_CYCLIC (curve) == true)) {
                ...
                vector_type tangent2
                        = (to_start_point == false) ? find_half_tangent(adjacent_curve, true, &n_points,
                                                                        tangent_surround) : find_half_tangent(
                                adjacent_curve, true, &n_points,
                                tangent_surround);
...
spline_linear_enough(spline_type *spline, curve_type curve,
    ...
    LOG3 ("  Line endpoints are (%.3f, %.3f, %.3f) and ", START_POINT(*spline).x, START_POINT(*spline).y,
          START_POINT(*spline).z);
